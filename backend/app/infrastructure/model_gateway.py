...
def build_model_gateway(settings: Settings) -> ModelGateway:
    ...
    Fake and real are chosen by exactly one flag (``mock_model_mode``): mock
    resolves to the deterministic ``FakeModelGateway``, non-mock to the real Qwen
    adapter. Both branches are explicit — an earlier revision silently degraded a
    non-mock configuration to the fake, which meant a deployment that had asked
    for the real model would receive plausible-looking heuristic output with no
    indication anything was wrong. A misconfiguration must fail loudly instead.

    ``Settings.validate_runtime_contract`` already rejects a non-mock
    configuration without a base URL and key, so the real branch can rely on
    both being present.
    """
    if settings.mock_model_mode:
        return FakeModelGateway()
    # Imported lazily so importing this module never requires the HTTP stack.
