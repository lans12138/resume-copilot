...
def build_embedding_gateway(settings: Settings) -> EmbeddingGateway:
    ...
    deterministic fake, non-mock to the real Qwen text-embedding adapter. The two
    factories previously disagreed — this one raised ``NotImplementedError`` while
    the chat one silently returned the fake — so a non-mock run would fail on
    embeddings but quietly produce fake extractions. Both now resolve the same way.
    """
    ...
    return build_qwen_embedding_gateway(settings)
