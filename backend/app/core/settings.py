...
class Settings(BaseSettings):
    ...
    # FIN-006: how long a committed-but-unpublished fact may sit before the
    # republish sweep re-delivers it, and how long a "live" run must be quiet
    # before it is treated as an abandoned claim (worker died holding the lock).
    # The lease window must comfortably exceed the slowest pass, or the sweep
    # would deliver a second execution over a run that is still making progress.
    republish_queued_after_seconds: int = Field(default=120, gt=0, le=86_400)
    run_lease_seconds: int = Field(default=900, gt=0, le=86_400)
    # FIN-006: orphan objects younger than this are never deleted; the window
    # covers uploads whose database row has not been committed yet.
    orphan_file_safety_window_minutes: int = Field(default=60, gt=0, le=100_800)
    orphan_file_scan_batch_size: int = Field(default=500, gt=0, le=10_000)
