...
``ApplicationRunService``; production stages a post-commit Celery resume while
hermetic domain tests can drive the same execution method inline.
"""
    ...
    async def decide(
        ...
        *,
        ...
        if new_status == ApprovalStatus.REJECTED:
            ...
            # APPROVED/EDITED: publish a checkpoint resume after commit (upstream).
            await self._run_service.emit_status(
                agent_run,
