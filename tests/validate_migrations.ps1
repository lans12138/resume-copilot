...
try {
    ...
    if ($revision -ne '0013_match_explanations') {
...
"@
    ...
    Write-Output (
        ...
        "revision=$revision tables=26 workflow_fks=7 vector_dimension=$vectorDimension readiness=ready degradation=503 recovery=ready"
    )
}
finally {
    & docker compose `
