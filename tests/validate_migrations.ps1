...
try {
    ...
    if ($revision -ne '0012_evaluation_tables') {
...
"@
    ...
    Write-Output (
        ...
        "revision=$revision tables=25 workflow_fks=7 vector_dimension=$vectorDimension readiness=ready degradation=503 recovery=ready"
    )
}
finally {
    & docker compose `
