...
def test_deferred_create_returns_created_and_publishes_only_when_flushed() -> None:
    asyncio.run(_deferred_create())


async def _deferred_create() -> None:
    class RecordingEnqueuer:
        def __init__(self) -> None:
            self.calls: list[dict[str, Any]] = []

        def enqueue_match_run(self, *args: Any, **kwargs: Any) -> None:
            raise AssertionError("wrong run type")

        def enqueue_application_run(self, run_id: UUID, **kwargs: Any) -> None:
    ...
    run, application_run = await service.create_application_run(actor, app.id)

    assert run.status is RunStatus.CREATED
    assert await service.get_application_run_detail(run.id) == (run, application_run, None)
    ...
    approval = await service.get_application_run_detail(run.id)
    first = approval[2]
    assert first is not None
    reread = await run_service.get_run(run.id)
    assert reread is not None and reread.status is RunStatus.WAITING_APPROVAL

    await service.decide_approval(
        actor, first.id, DecisionAction.APPROVE, expected_version=first.version
    )
    reread = await run_service.get_run(run.id)
    assert reread is not None and reread.status is RunStatus.RUNNING
    service.publish_pending_deliveries()
    assert len(enqueuer.calls) == 2
    resume = enqueuer.calls[1]
    assert resume["intent"] is ExecutionIntent.RESUME
    assert resume["approval_id"] == first.id
    assert isinstance(resume["resume_version"], str)
