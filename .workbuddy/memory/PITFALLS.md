...
- **PowerShell 里给 git 传多个路径别用逗号数组**：`git add -- a.md, 'b.md'` 中 `a.md, b.md` 被解析成**一个数组实参**传给 git，变成单个带逗号的 pathspec → exit 128。每个路径单独引号、空格分隔。
- **打 tag 的完整流程（绕吞 ref）**：commit 后 `rev-parse HEAD` → 手写 SHA 到 `.git/refs/tags/v1.0.0` → `rev-parse refs/tags/v1.0.0` 验证落盘 → python subprocess `git push origin refs/tags/v1.0.0` → `ls-remote origin refs/tags/v1.0.0` 验远端。注意顺序：**先 commit 成功再写 tag**，写早了会指向旧 HEAD（2026-09-16 踩过，幸好 push 前发现）。
...
- **后台长驻进程活不过 2 分钟**：`run_in_background` 的命令（哪怕直接是 python）也在 ~121s 被宿主超时杀（两次实测 watcher 死于 2m01s，stderr 为空）；`Start-Process` 分离进程同样存活不下来。**要等长任务别派守护进程**——写成一次性轮询脚本，由自己反复短调用（每次 sleep N + 执行 + 读结果落盘）。
